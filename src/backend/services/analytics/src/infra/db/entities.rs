...
pub mod saved_queries {
    ...
    //!
    //! Metadata in the service DB, mirroring `metrics`. `sql` holds a single
    //! `SELECT`/`WITH` over the contract, validated by the query gate on write
    //! and run; CRUD never reaches ClickHouse.
    use sea_orm::entity::prelude::*;
