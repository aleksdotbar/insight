...
echo "==> stand: namespace=$NAMESPACE release=$RELEASE"
...
kubectl -n "$NAMESPACE" get configmap "$platform_cm" >/dev/null 2>&1 || die \
...
cm_value() {
  kubectl -n "$NAMESPACE" get configmap "$platform_cm" -o "jsonpath={.data.$1}"
}
...
if ! kubectl -n "$NAMESPACE" get secret "$ir_secret" -o name >/dev/null 2>&1; then
...
if [[ -z "$IMAGE" || -z "$PULL_SECRETS" ]]; then
  ...
  release_values="$(helm get values "$RELEASE" -n "$NAMESPACE" -a -o json 2>/dev/null || true)"
...
if [[ -z "$DB_SECRET" ]]; then
  ...
  for candidate in insight-db-creds "${RELEASE}-db-creds"; do
    have_maria="$(kubectl -n "$NAMESPACE" get secret "$candidate" \
      -o 'jsonpath={.data.mariadb-password}' 2>/dev/null || true)"
    have_ch="$(kubectl -n "$NAMESPACE" get secret "$candidate" \
...
for _ in $(seq 1 60); do
  phase="$(kubectl -n "$NAMESPACE" get pod -l "job-name=$job_name" \
...
while [[ "$SECONDS" -lt "$deadline" ]]; do
  ...
  if [[ "${failed:-0}" -ge 1 ]]; then
    ...
    echo "         kubectl -n $NAMESPACE logs job/$job_name" >&2
    exit 1
  fi
  ...
  waiting="$(kubectl -n "$NAMESPACE" get pod -l "job-name=$job_name" \
...
echo "         kubectl -n $NAMESPACE describe job/$job_name" >&2
exit 1
