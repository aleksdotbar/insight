...
def _dev_lead_login_id(stand_manifest: Manifest) -> tuple[str, str]:
    ...
    `get_idp_source_type`: every persona's external id is their own roster
    uuid (`keycloak_realm` pins each realm user's Keycloak id to it, and
    Keycloak issues `sub` verbatim). `capabilities.idp` is the identity
    source_type those rows were written under, so it is usable directly as
    `source_type` here.
    """
    source_type = stand_manifest.capabilities.idp
    person = stand_manifest.fixture("dev_lead")
    return source_type, person.uuid
