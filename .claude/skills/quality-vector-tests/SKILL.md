...
- **The denominators.** "API coverage — 100%" is unfalsifiable until you know 100% of what. Count
  ...
  `.claude/skills/quality-vector-tests/scripts/counts.sh` (connectors, catalog metrics, dbt models and tests, specs). Those counts go in
...
- **Every How measured carries its denominator.** `n of m`, not "some". The shapes:
  *<all> of <all> catalog metrics*, *<n> of <n> acceptance criteria in the issue*, *<all> of <all>
  ...
  `.claude/skills/quality-vector-tests/scripts/counts.sh` at the moment you write the target — never copy one out of
...
`.claude/skills/quality-vector-tests/scripts/counts.sh` before using it.
...
only inside labelled snapshots — the table below and both worked examples —
where the point is the shape of a finished target, not the value. The numbers
move, and the last time they moved the failure was silent: the catalog used to live inline in
