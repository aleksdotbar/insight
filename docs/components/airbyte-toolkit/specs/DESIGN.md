...
| `cpt-insightspec-adr-cdk-prebuilt-images` | **(SUPERSEDED by ADR-0016 2026-05-21)** CDK connectors carry the full Docker image reference in the descriptor; reconcile splits it into `dockerRepository`/`dockerImageTag` per Docker reference grammar — no derivation, no convention, no env var; never builds at runtime. The location of the ref moved from top-level `descriptor.cdk_image` to `descriptor.images.cdk.image` per ADR-0016. | §3.2 reconcile-engine, §3.9 Reconciliation Model |
| `cpt-insightspec-adr-enrich-image-in-descriptor` | **(SUPERSEDED by ADR-0016 2026-05-21)** enrich sidecar image declared in the descriptor; sole source of truth; Helm/CI/dev-up read from descriptor, never from chart values. The location of the ref moved from top-level `descriptor.enrich_image` to `descriptor.images.enrich.image` per ADR-0016. | §3.2 reconcile-engine, §3.13 Argo Integration |
...
| `cpt-insightspec-adr-descriptor-images-block` | every connector with a `Dockerfile` declares all its images under a map-style `descriptor.yaml.images:` block keyed by free-form keys (`cdk`, `enrich`, …); each entry has `name`/`dockerfile`/`context`/`image`; no top-level `cdk_image:` / `enrich_image:` fields exist; reconcile reads `images.cdk.image` and `images.enrich.image`; CI uses dynamic discovery (single matrix job scans descriptors, fan-out builds each entry, separate commit patches `images.<key>.image` after build); supersedes ADR-0011 and ADR-0014 | §3.2 reconcile-engine, §3.13 Argo Integration |
...
**CDK connector lifecycle** (per ADR-0011): Insight CDK connectors point at pre-built Docker images via the descriptor field `cdk_image`, which carries the full image reference (e.g. `ghcr.io/constructorfabric/source-bitbucket-cloud-insight:2026.04.21.16.10-b36cf42`). Reconcile splits this string into `dockerRepository` + `dockerImageTag` using the canonical Docker reference grammar (digest `@sha256:` first; else last `:` after last `/`; no tag → `:latest`) — no derivation, no convention, no env var. The image name has no required structure. `descriptor.version` is the Insight semantic version (audit, Argo labels) and is independent of `cdk_image`. Reconcile **never builds Docker images at runtime** — it only registers/updates the source_definition. First registration uses `source_definitions/create_custom`; subsequent image bumps (same repository, new tag) use `source_definitions/update` (existing `ab_set_definition_image_tag`). Missing `cdk_image` for `type=cdk` → reconcile WARNs and skips that connector for the run. A **repository** change cannot be applied by `source_definitions/update`, whose payload carries only `dockerImageTag`. Reconcile acts on exactly one such change: a live definition still on `airbyte/source-declarative-manifest`, which is what a connector rewritten from a declarative manifest to a CDK source looks like, since the manifest-backed definition keeps the connector's name. Any other repository difference — a registry move, a descriptor typo — stays a WARN and skip, because tearing a live source down for either would be out of all proportion. Reconcile migrates instead (`reconcile_migrate_definition_kind`): export and persist per-connection state, delete the sources bound to the old definition, delete the definition, register the CDK image under the freed name, and let the source and connection stages rebuild from the descriptor and Secret; the connection stage restores the exported state onto the replacement. Rebuilding the source config from the Secret rather than copying the old one is deliberate — it drops any config field the rewrite retired, which a verbatim copy would carry into a spec that no longer accepts it. The order is delete-then-create because the definition name is the identity; a crash between the two leaves no definition, which the next pass handles as a first publish. Local-dev image build remains in `lib/cdk-build.sh` (operator-invoked, not part of the reconcile loop).
...
| `connectors/<name>/descriptor.yaml` | `enrich_image` field (optional) | full Docker reference (e.g. `ghcr.io/constructorfabric/insight-jira-enrich:1.2.3`) | Single source of truth for the connector's enrich sidecar image (per ADR-0014); reconcile passes it to the `ingestion-pipeline` submission |
