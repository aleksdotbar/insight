...
Consistent with project convention, the Confluence connector uses a nocode YAML manifest (Airbyte DeclarativeSource). All other connectors in the project (m365, bamboohr, claude-api, jira) follow this pattern. Features that cannot be expressed declaratively (email resolution via batch User API lookup, version-to-activity row expansion) are deferred to downstream dbt layers rather than falling back to CDK Python in Phase 1.
