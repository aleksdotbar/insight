...
Consistent with project convention, the Jira connector uses a nocode YAML manifest (Airbyte DeclarativeSource). All other connectors in the project (m365, bamboohr) follow this pattern. Features that cannot be expressed declaratively are deferred to future iterations or downstream layers rather than falling back to CDK Python in Phase 1.
