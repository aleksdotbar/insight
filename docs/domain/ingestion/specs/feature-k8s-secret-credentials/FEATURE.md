...
| `cpt-insightspec-actor-ing-toolbox` | Runs `src/ingestion/reconcile-connectors/main.sh` inside the cluster, reads Secrets |
...
**Touches**:
- Script: `src/ingestion/reconcile-connectors/main.sh` (secret discovery in `lib/secrets.sh`)
...
The example tenant config **MUST** be updated to remove inline credentials, showing only `tenant_id` and non-credential connector fields (e.g., `start_date`). A comment **MUST** reference K8s Secrets as the credential source.

**Touches**:
- example tenant config (no longer present in the tree)
