...
- Auto-initialization on `cd deploy/gitops && make deploy ENV=local` — no manual setup
...
- **Purpose**: Provide a fully automated local development environment via Kind K8s cluster that mirrors production topology. Running `cd deploy/gitops && make deploy ENV=local` creates a working instance with ClickHouse, Airbyte, Argo Workflows, and all initialization — no manual configuration required.
...
- **Scope**:
  ...
  - Applied by `./reconcile-connectors.sh` (and the in-cluster reconcile CronWorkflow)
