...
- Service access via edge routes (PR #224): the umbrella chart originally configured `ingress-nginx` Ingresses for Frontend, API Gateway, Airbyte UI and Argo UI; today a single Gateway API HTTPRoute sends the edge to the gateway (which fans out to the SPA and APIs), while the Airbyte and Argo UIs are reached by port-forward. On a local Kubernetes cluster the Kind config maps host ports 80/443 to the edge proxy; direct port-forwards (`kubectl port-forward`) remain available for debugging.
