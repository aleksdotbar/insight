...
| `cpt-bronze-to-api-e2e-nfr-parallel-safe` | Zero cross-worker contamination | `session-rig` + `ch-seeder` + `dbt-runner` | Per-worker schema suffix (`_w0`, `_w1`, …); analytics binary is read-only against per-worker schemas | `src/ingestion/tests/e2e/meta/test_dbt_runner.py` runs 100 randomized parallel pairs; CI fails on any contamination |
| `cpt-bronze-to-api-e2e-nfr-diff-readability` | Cell-precise diff in pytest captured stdout | `csv-asserter` | Custom `assert_frame_equal` wrapper that catches the assertion, formats first 20 mismatches as `(key, column, expected, actual)` lines, raises a new `AssertionError` with that text — visible in pytest report | `src/ingestion/tests/e2e/meta/test_expect_engine.py` injects a known mismatch and asserts the captured stdout contains the expected (key, column) pairs |
