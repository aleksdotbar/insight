...
- **#1664** — a duplicate admin-threshold create (`POST /v1/admin/metric-thresholds`) answers 500 (an unmapped UNIQUE-constraint violation) instead of the declared 409; `test_create_409_duplicate` (`tests/stand/api/identity/test_admin.py`) pins the contract as a strict xfail, and `409` is excluded via `BLOCKED["POST /v1/admin/metric-thresholds"]`.
