...
Invariants: builtins are read-only through this API; custom SQL is single-SELECT gated, must emit the observation contract, and executes as `presentation_ro`; export/import identity is `metric_key` (not the tenant-scoped row id), so a graph re-homes cleanly on import. The saved-query console (`/v1/queries*`) is a separate surface and is unchanged.
