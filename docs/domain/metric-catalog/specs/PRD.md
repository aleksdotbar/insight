...
- **v1.11** (current): Two operational changes from product-owner notes. **(1)** Read endpoint switched to `POST /v1/catalog/get_metrics` with parameters in the JSON body. The previous form used the GET verb on the same resource path with `role_slug` / `team_id` as query-string parameters. Rationale: keeps `role_slug` / `team_id` (and any future request context) out of HTTP access logs, proxies, and analytics where query strings are commonly captured. Practical consequence: HTTP/CDN caching no longer applies; the cache layer (`cpt-metric-cat-fr-cache`) is now explicitly **server-side** (Redis or equivalent shared cache, or pub-sub-coordinated in-process), implemented as a deliberate code path keyed by `(tenant_id, role_slug, team_id)`. Functional semantics, response shape, and resolution behavior are unchanged. The closed OQ on "alerting endpoint variant" updated to drop the CDN-caching rationale (no longer load-bearing under POST). **(2)** Metadata text columns ship in v1 as plain-English strings — `label`, `sublabel`, `description` — instead of the previously-planned i18n-key triplet (`label` + `_i18n_key` suffix, and the same for sublabel and description). Rationale: i18n is explicitly out of v1 scope and shipping i18n-key references without an i18n loader on either end is dead weight. Future i18n work lands additively as a separate translation table keyed by `(metric_key, locale)` without touching v1 columns; the v1 `label` row becomes the source string for the default locale. NFR Exclusion §6.2 reworded accordingly; the source-of-truth OQ and the per-tenant-localization OQ in §13 are reframed under the new "plain English now, additive i18n later" model. Dependencies table drops the frontend i18n loader row (no v1 dependency); Assumptions drops the "i18n keys resolved by consumers" clause.
- **v1.10**: Reframed tenant-custom metrics from "out of scope, cross-layer PRD required" to "additive follow-on, v1 schema pre-wired". **`metric_catalog` gains a nullable `tenant_id` column in v1** with a CHECK constraint forcing `tenant_id IS NULL` — identical pattern to `lock_expires_at`. All v1 rows remain global (product-owned); the column ships dead, ready for the follow-on FR that lifts the CHECK and adds CRUD for tenant-custom metrics. This chooses path (a) from the v1.9 §13 OQ (nullable `tenant_id` on the existing table) over path (b) (separate `metric_catalog_tenant_custom` table). Rationale: architectural symmetry with thresholds — product-owned global baseline + tenant-owned additive overlay — without the footgun of making product metadata tenant-editable. Cross-tenant comparability preserved because product-owned metrics stay product-owned (`cpt-metric-cat-fr-metadata-writes` unchanged); tenant customization becomes additive (append a row with `tenant_id = X`), never a replacement. **§4.2 bullet on tenant-custom rewritten** from "warrants its own PRD" to "ships in a follow-on PRD; v1 schema ready". **§13 OQ on tenant-custom closed**: path (a) chosen. **Three new §13 OQs opened for the follow-on scope**: (i) query-layer storage model for tenant-custom — tenant-scoped rows in `analytics.metrics` vs a formula DSL poverh existing metrics (prototype demonstrated both UX shapes) vs both; (ii) disable-for-me slot — additive table letting a tenant hide a product-owned metric for themselves without affecting others; (iii) PK evolution strategy — keep v1 `PK = metric_key` and rebuild when tenant-custom lands, or ship v1 with synthetic `id` + `UNIQUE(tenant_id, metric_key)` to avoid PK rebuild. **New §11 Assumption** and **§12 Risk** on tenant maintenance burden: when tenant-custom lands, product commits silver backward-compat only for metrics in the global catalog; tenants owning custom metrics accept responsibility for updating them when depended-upon silver columns change. This is the contract that makes silver evolution tractable — append-only-forever obligation on product-owned silver coverage only, not on every field a tenant might have bound to. Read endpoint response shape is unchanged in v1; when the follow-on lifts the CHECK, resolution filters rows by `tenant_id IN (caller, NULL)` and optionally surfaces `owner: 'product' | 'tenant'` as an additive field.
...
It replaces the status quo where metric metadata is duplicated between the frontend (`api/thresholdConfig.ts` in the screensets frontend (separate repository) with `BULLET_DEFS`, `IC_KPI_DEFS`, and most of `METRIC_KEYS`) and backend seed migrations. The catalog is an independent primitive: its first consumer is Dashboard Configurator, but it is not coupled to any specific visualization surface, and future consumers (alerting, reports, scheduled digests, admin audits) read the same response shape.
...
**Role**: Serves the catalog via `POST /v1/catalog/get_metrics` with resolved per-tenant thresholds. Exposes admin CRUD for thresholds. Validates metadata integrity at the API layer.

#### Catalog Consumer

**ID**: `cpt-metric-cat-actor-consumer`

**Role**: Any backend or frontend component that reads `POST /v1/catalog/get_metrics` and relies on the returned metadata. In v1 this is Dashboard Configurator; in later waves, it will include alerting, reports, scheduled digests, and the admin UI.
...
None beyond project defaults (Rust, Axum, SeaORM, MariaDB). The catalog inherits the analytics service runtime.

## 4. Scope

### 4.1 In Scope

- `metric_catalog` table: one row **per `metric_key`** (all rows global in v1) with fields `tenant_id` (nullable, v1 CHECK forces `NULL`), `label`, `sublabel`, `description`, `unit`, `format`, `higher_is_better`, `is_member_scale`, `source_tags` (array of strings), `is_enabled`, timestamps. Per-tenant metadata overrides on product-owned metrics are out of v1 scope; if ever needed, an additive `metric_catalog_tenant_override` table lands without schema change to `metric_catalog`.
...
- `POST /v1/catalog/get_metrics` read endpoint returning catalog + resolved thresholds for a request's `(tenant, role, team)` context
...
- Cache layer (configurable TTL, default 5 minutes) fronting `POST /v1/catalog/get_metrics`
...
The system **MUST** persist rows in `metric_catalog` with at least: `tenant_id` (nullable, v1 CHECK enforces `IS NULL`), `metric_key`, `label`, `sublabel`, `description`, `unit`, `format`, `higher_is_better`, `is_member_scale`, `source_tags` (array of strings), `is_enabled`, `created_at`, `updated_at`. In v1, because of the CHECK, every row has `tenant_id IS NULL` and represents a global product-owned metric — functionally "one row per `metric_key`", identical to v1.9 behavior. The **primary key evolution strategy** (keep `PK = metric_key` for v1 and rebuild when tenant-custom lands vs ship v1 with synthetic `id BIGINT AUTO_INCREMENT` + `UNIQUE(tenant_id, metric_key)` treating NULL-global and non-NULL tenant rows as distinct) is a DESIGN question — see §13 OQ. Either option produces identical v1 external behavior. In v1 rows are created by seed migrations; the table shape is intentionally friendly to future population from an external source (e.g., silver-plugin manifests registering their metrics at install time) without requiring a schema change.
...
The system **MUST** honor `is_enabled = false` by excluding the row from `POST /v1/catalog/get_metrics`. Existing consumers (e.g., dashboards) referencing a disabled metric **MUST** continue to work — the reference resolves as absent rather than erroring — so downstream systems can tolerate deprecations without coordinated rollbacks.
...
#### POST /v1/catalog/get_metrics Returns Fully Resolved Catalog

- [ ] `p1` - **ID**: `cpt-metric-cat-fr-read-endpoint`

The system **MUST** expose `POST /v1/catalog/get_metrics` returning, for the caller's tenant, every `is_enabled = true` catalog row joined with its resolved thresholds per `cpt-metric-cat-fr-scoped-thresholds`. The endpoint **MUST** accept a JSON request body with optional fields `role_slug` and `team_id`; when present, they participate in threshold resolution. When absent (or when the body is empty `{}`), resolution falls back to the `tenant` / `product-default` chain. POST is chosen over GET deliberately so that `role_slug` / `team_id` (and any future request-context fields) never appear in HTTP access logs, proxy logs, or third-party analytics that commonly capture query strings. The response **MUST** include a `tenant_id` echo and a `generated_at` timestamp to support client-side cache reasoning. The response **MUST** also include, per metric, the scope that supplied the resolved threshold as `resolved_from: "team+role" | "team" | "role" | "tenant" | "product-default"` so consumers and admin tools can explain which row won without a second request.

**Rationale**: Consumers need one call to hydrate everything they need about every metric they will render or evaluate. Round-trips per metric would be unacceptable. Surfacing the winning scope by default costs a single short string per metric and closes the "why is this color different here" debugging loop without a second endpoint. The body fields are optional so generic catalog hydrators (e.g., an admin audit UI listing the tenant-wide state) keep working without knowing about role or team. POST also makes server-side caching (`cpt-metric-cat-fr-cache`) the only viable cache layer — HTTP / CDN caches do not cache POST bodies — which the catalog already requires for cross-replica invalidation correctness (`cpt-metric-cat-nfr-cross-replica-invalidation`), so no behavior is lost by moving the verb.
...
The system **MUST** front the read endpoint with a **server-side** cache layer whose default TTL is 5 minutes and whose cache key includes at minimum the tenant identifier (plus `role_slug` / `team_id` when supplied in the body). Admin writes **MUST** invalidate the cache for the affected tenant so threshold changes appear on the next page load. **HTTP-level and CDN-level caching are not in scope** — `POST /v1/catalog/get_metrics` is not cacheable by HTTP intermediaries, and the read endpoint deliberately uses POST to keep `role_slug` / `team_id` out of access logs (`cpt-metric-cat-fr-read-endpoint`). The remaining cache-implementation choice (shared Redis vs pub-sub-coordinated in-process) is a DESIGN decision tracked in §13 OQ and bounded by `cpt-metric-cat-nfr-cross-replica-invalidation`; this FR mandates that a cache exists and that its invalidation correctly tracks admin writes.
...
- The analytics **structured log stream** — for real-time observability, alerting, and operational dashboards.
...
The system **MUST** seed the initial catalog by importing the metadata currently hardcoded in `api/thresholdConfig.ts` in the screensets frontend (separate repository) (`BULLET_DEFS`, `IC_KPI_DEFS`) and `types/index.ts` in the screensets frontend (separate repository) (`METRIC_KEYS`). The seed is a one-time export that the product team reviews before merging. After the seed lands and the frontend consumes `POST /v1/catalog/get_metrics`, the duplicated metadata on the frontend **MUST** be removed. The rollout **SHOULD** go through a short transitional phase where the frontend fetches the catalog but retains local metadata as a fallback; the removal of the fallback ships in the immediately following release. Byte-for-byte comparison between rendered output before and after migration is a rollout gate.
...
`POST /v1/catalog/get_metrics` p95 response time **MUST** be ≤ 100ms on a cache hit and ≤ 500ms on a cache miss for a tenant with up to 200 catalog rows.

**Threshold**: p95 ≤ 100ms (hit), ≤ 500ms (miss), measured at the analytics service level.

#### Consistency — Write-After-Read Visibility

- [ ] `p1` - **ID**: `cpt-metric-cat-nfr-write-visibility`

A successful threshold write **MUST** be visible to subsequent `POST /v1/catalog/get_metrics` calls from the same tenant within one cache TTL, with no manual invalidation required.
...
**Description**: Returns the catalog for the caller's tenant, filtered to `is_enabled = true`, with resolved thresholds per the precedence rule in `cpt-metric-cat-fr-scoped-thresholds`. Request body (JSON, all fields optional): `{ role_slug?: string, team_id?: string }` — participate in threshold resolution when supplied; empty body `{}` resolves at the `tenant` / `product-default` chain only. POST is deliberate so request-context fields stay out of HTTP access logs. Response shape: `{ tenant_id, generated_at, metrics: [{ id, metric_key, label, sublabel, description, unit, format, higher_is_better, is_member_scale, source_tags: [string], schema_status, schema_error_code?, thresholds: { good, warn, alert_trigger?, alert_bad?, resolved_from, bounded_by_lock } }], links: [{ query_id, catalog_metric_ids: [Uuid] }] }`. `id` (UUIDv7) is the stable wire lookup identifier consumers MUST key off; `metric_key` is additionally surfaced per **ADR-002** as the FE-bridge identifier for the catalog-hydration transitional release (constructorfabric/insight-front#66). `links` carries the time/filter-invariant `metric_query_catalog` mapping (ADR-001 junction) per **ADR-003** so consumers cache the Layer-2 query→catalog map for the same TTL as the catalog. `source_tags` is always an array; single-source metrics return a one-element array. The `resolved_from` string identifies which scope supplied the threshold row (v1 domain: `"team+role"`, `"team"`, `"role"`, `"tenant"`, `"product-default"`); `bounded_by_lock` is a separate boolean signal indicating whether the walk halted on a locked broader-scope row before reaching the most-specific candidate.
...
**Protocol/Format**: Consumers **MUST** fetch `POST /v1/catalog/get_metrics` once per session (or per cache-TTL window), key metadata lookups by the stable wire identifier `id` (UUIDv7), and degrade gracefully when a previously-seen `id` is absent from a later response. Consumers **MUST NOT** hardcode metric metadata that the catalog provides. `metric_key` is additionally surfaced per **ADR-002** as the FE-bridge identifier for the catalog-hydration transitional release; consumers **SHOULD NOT** use it as their primary lookup key — that contract belongs to `id`. The top-level `links` array (**ADR-003**) exposes the time/filter-invariant `(query_id → catalog_metric_ids)` mapping so consumers can cache the query→catalog correspondence at the same TTL as the catalog rather than re-deriving it per value request.
...
5. Next `POST /v1/catalog/get_metrics` for the tenant returns the new threshold
...
**Preconditions**: The new metric's `metric_query` is either being added in the same PR or already exists in `analytics.metrics`. Plain-English `label`, `sublabel`, and `description` strings are authored in the same migration.
...
4. `POST /v1/catalog/get_metrics` returns the new entry after cache expiry or invalidation
...
1. Consumer calls `POST /v1/catalog/get_metrics`
...
- [ ] `metric_catalog` and `metric_threshold` tables exist in analytics's MariaDB, with sea-orm migrations that seed the frontend metadata one-to-one
...
- [ ] Admin write on replica A makes the change visible to `POST /v1/catalog/get_metrics` served by replica B within `cpt-metric-cat-nfr-cross-replica-invalidation`'s threshold
...
- [ ] `POST /v1/catalog/get_metrics` accepts optional `role_slug` and `team_id` fields in the JSON request body and resolves thresholds per the precedence `team+role → team → role → tenant → product-default`
- [ ] `POST /v1/catalog/get_metrics` surfaces `resolved_from` per metric threshold (value in `{ team+role, team, role, tenant, product-default }`) so consumers can explain which scope supplied each value
...
- [ ] `POST /v1/admin/metric-thresholds` persists a new threshold at any v1 scope and invalidates the tenant's cache; the value is visible to the next `POST /v1/catalog/get_metrics` for the matching `(tenant, role, team)` context without waiting for TTL
...
| Dashboard Configurator PRD (not yet written) | Defines `role_catalog` (per-tenant role taxonomy) and a team-identifier convention. v1 of this catalog does not block on it: `role_slug` / `team_id` are string references without FK. The follow-on migration that adds FK constraints onto `role_catalog` and the future team-catalog is gated on Dashboard Configurator's delivery. | p2 (future) |

## 11. Assumptions

- Tenants are single-region; the catalog does not need multi-region replication.
- v1 stores plain-English strings in `label`, `sublabel`, `description` and ships no i18n loader. Localization is a follow-on (see §6.2 NFR Exclusions).
...
| Threshold override precedence misunderstood by consumers | Inconsistent bullet colors across surfaces for the same metric | `cpt-metric-cat-fr-scoped-thresholds` fixes precedence; the `POST /v1/catalog/get_metrics` response always returns `resolved_from` so the winning scope is visible without a diagnostics call |
...
| Team-scoped threshold written by a team lead contradicts a tenant admin's rollout intent | Org-wide comparability fragments silently across teams | The `POST /v1/catalog/get_metrics` response surfaces `resolved_from`, so tenant admins can audit which teams have overrides and why; governance on team-scoped thresholds (justification, expiry, caps) is tracked as an OQ and revisited once the first team-lead authoring surface lands |
...
| ~~Source of truth for `label` strings — backend seed migration vs frontend i18n loader?~~ **Closed v1.11**: moot under the plain-English-now model. v1 stores authoritative `label` / `sublabel` / `description` strings on the catalog row (seeded by the backend migration; consumed verbatim by every reader). When i18n lands as a follow-on, the additive translation table is the source of truth for non-default locales; v1 `label` becomes the default-locale source string without column change. | Insight Product Team + Frontend tech lead | Closed v1.11 |
...
| Per-tenant metadata localization — once i18n lands as a follow-on, do tenants get a way to override translations or labels for their own organization (e.g., "we call this metric 'Story Points', not 'Tasks Closed'")? Trade-off: cross-tenant comparability vs tenant-specific terminology. v1 is plain-English-only and product-team-owned; this question is dormant until i18n ships. | Insight Product Team | Before the second tenant onboards or at i18n-program kickoff, whichever comes first |
| ~~Alerting consumer contract — will alerting use the same read endpoint or a narrower variant?~~ **Resolved**: alerting and visual thresholds share the `metric_threshold` table, distinguished by kind. The alerting subsystem (separate domain) consumes the same table filtered to alerting kinds. If a narrower endpoint is later useful, it lands as a path-based variant (e.g., `POST /catalog/alerting/get_metrics`) rather than a body-parameter switch — keeps cache keys, audit trails, and request routing cleanly separated, and avoids mode-flag overloading of the primary read endpoint. | Insight Product Team | Closed |
...
This section is informative, not normative. It captures the delta between today's backend code on constructorfabric/insight and what this PRD requires, so DESIGN can scope work accurately.

### Present

- `services/analytics` is a running Rust / Axum service with a working `/v1/metrics`, `/v1/metrics/{id}/query`, `/v1/thresholds`, `/v1/persons/{email}`, `/v1/columns` surface. SeaORM entities cover `metrics`, `thresholds`, `table_columns`. Sea-orm migrations run at service startup.
...
- Frontend `api/thresholdConfig.ts` in the screensets frontend (separate repository) defines `BULLET_DEFS` and `IC_KPI_DEFS`; `types/index.ts` in the screensets frontend (separate repository) defines `METRIC_KEYS`. These are the canonical sources of current metadata and the seed input for `cpt-metric-cat-fr-seed-from-frontend`.
...
- No cache layer in analytics dedicated to catalog reads; the shape of the layer (in-process vs shared) is a DESIGN question.
...
| **#223 — gold views honest-null contract** (merged) | Unblocks the "no `null%` / `nullh` in rendered DOM" story for consumers of `POST /v1/catalog/get_metrics`. |
...
3. Analytics-api: add `POST /v1/catalog/get_metrics` (with server-side cache, optional `role_slug` / `team_id` body fields, and `resolved_from`) plus `POST/PUT/DELETE /v1/admin/metric-thresholds` accepting all v1 scopes.
4. Frontend: hydrate from `POST /v1/catalog/get_metrics` alongside existing local metadata as fallback; ship a release that introduces this dual-path and passes the current viewer's `(role, team)` in the request body where known.
