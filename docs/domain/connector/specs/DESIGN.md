...
**Location**: [`docs/components/connectors/`](../../../components/connectors/)
...
| `src/ingestion/reconcile-connectors/main.sh` | Shell script reading `descriptor.yaml` | Provisions Airbyte connections from descriptor metadata |
...
3. `src/ingestion/reconcile-connectors/lib/secrets.sh` discovers secrets by label and injects credentials into Airbyte
...
| Duplicate definitions in Airbyte | Earlier registration created a new definition each time | Fixed: updates definition in-place, ID stored in the recorded Airbyte definition IDs |
...
| State out of sync | Recorded IDs do not match Airbyte | Re-run `src/ingestion/reconcile-connectors/main.sh` to re-resolve all IDs |
...
3. **Upload**: `src/ingestion/reconcile-connectors/main.sh` — updates the definition in-place and records its ID
...
7. **State recovery**: re-run `src/ingestion/reconcile-connectors/main.sh` if recorded IDs are missing or stale
