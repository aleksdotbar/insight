...
* A correction writes binding observations only. The journal does not encode which identity value belongs to which account (`persons` rows carry the source instance, not an account discriminator); that linkage lives in the `identity_inputs` evidence (`source_account_id` on every row), which the resolver and the value-addressed bulk bind consume. E-mail resolution therefore follows corrections indirectly, through the current bindings of the accounts observing each e-mail (enabler 1).
...
4. **Unique per-row observation timestamps** — the natural key carries no account discriminator, so two `id` observations for two accounts of one source, bound to one person at the same `created_at`, would collide and `INSERT IGNORE` would drop one. The write path allocates strictly increasing `DATETIME(6)` timestamps per affected row within an operation (bulk included); extending the key with an account discriminator is a candidate follow-up migration.
