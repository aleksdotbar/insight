...
| Surface what needs operator attention | Derived review queue: contested-binding groups + accounts pending a decision; no status tables (planned, this iteration) |
...
- Read surface: the derived review queue (contested-binding groups + accounts pending a decision, with candidates), per-account binding history (explain), per-person account listing (matching table).
...
| `GET` | `/v1/resolution/attention` | Derived review queue: accounts pending a decision + contested-binding groups, with candidates and counts | planned |
...
5. Group accounts by normalized current e-mail; resolve each group in priority order (mirrors the .NET resolver; `domain/seed.rs::resolve_assignments`):
   ...
   - **No e-mail, or all profiles closed**: skip. E-mail remains the sole identity anchor for this seed.
...
| `alias_gdpr_deleted` | GDPR erasure archive | Future — with the GDPR purge flow |
...
3. GitHub `42`, GitLab `17` — unknown, no e-mail observed → skipped (MVP limitation; see #1776 and the review-queue open question in #2180).
...
| Pending-decision count | Review queue counts (planned, `/v1/resolution/attention`) | Accounts and contested-binding groups awaiting an operator decision | Backlog growth week-over-week |
