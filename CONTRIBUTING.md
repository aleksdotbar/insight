...
Run a supported Rust service directly from the bind-mounted workspace with
`cargo watch`:

```bash
./dev-compose.sh up --watch=analytics
./dev-compose.sh up --watch=analytics --no-frontend
```

Compose continues to own the service, network, dependencies, auth, and ports.
Cargo registry, git, and target directories use named volumes, so subsequent
debug builds are incremental. A watched service cannot also use `--from-ghcr`
or an image override. Currently supported: `analytics`.
